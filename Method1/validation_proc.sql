...
var rs_tables = snowflake.createStatement({
	sqlText: "SELECT DISTINCT upper(database_name) AS DATABASE_NAME, UPPER(schema_name) AS SCHEMA_NAME, UPPER(table_name) AS TABLE_NAME FROM " + VALIDATION_DB + "." + VALIDATION_SCHEMA + "." + VALIDATION_TABLE
}).execute();
//BEGIN TABLE LOOP
while (rs_tables.next()) {
	...
	var stmt_getdb_list = snowflake.createStatement({
		sqlText: "SELECT * from " + VALIDATION_DB + "." + VALIDATION_SCHEMA + "." + VALIDATION_TABLE + " where table_name ILIKE '" + table_name + "';"
	});
